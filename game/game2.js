...
let WIDTH = 600; // Initial value
let HEIGHT = 600; // Initial value

let scale = window.devicePixelRatio || 1;

// Scale the drawing context to match the device's pixel ratio
...
const resizeCanvas = () => {
    ...
    // Set the actual size of the canvas in pixels
    ...
    // Update WIDTH and HEIGHT used in the game logic
    WIDTH = width;
    HEIGHT = height;
};
...
function enableResponsiveCanvas() {
    // Adjust canvas CSS to make it responsive
    canvas.style.width = '100%';
    canvas.style.height = 'auto';
    canvas.style.maxWidth = '100%'; // Ensure it fills the width
    canvas.style.aspectRatio = '1 / 1'; // Keep it square

    // Adjust the canvas size and scaling
    const resizeCanvas = () => {
        const scale = window.devicePixelRatio || 1;
...
document.getElementById('phoneButton').addEventListener('click', () => {
    ...
    enableResponsiveCanvas(); // Make the canvas responsive
